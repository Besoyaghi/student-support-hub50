      ...
      if(paperResult.ok && paperResult.papers.length){
        ...
        paperResult.papers
          ...
          .forEach(p => {
            ...
            if(!grouped.has(key)) grouped.set(key, {bookId:p.bookId, title, firstNum:p.num || 9999});
            else grouped.get(key).firstNum = Math.min(grouped.get(key).firstNum, p.num || 9999);
          });

        const byBook = {};
        Array.from(grouped.values()).forEach(group => {
          if(!byBook[group.bookId]) byBook[group.bookId] = [];
          byBook[group.bookId].push(group);
        });

        const backendChapters = Object.entries(byBook).flatMap(([bookId, groups]) =>
          groups
